...
use crate::parsing::ast::{EnumVariant, Expr, Stmt, TypeMention, TypedName};
...
peg::parser! {
    pub grammar program_parser<'a>() for [&'a Token] {
        ...
        rule var_decl_stmt() -> Stmt =
            [t!(Var)] n:typed_name() e:assignment_right_side()?
        ...
        rule function_decl_stmt() -> Stmt =
            [t!(Def)] n:name() args:maybe_arguments_and_equals() body:expr() {
                Stmt::FunctionDeclaration{name:n, args: args.0, vararg: args.1, body, returns: args.2}
            }
        ...
        rule struct_no_token() -> (Token, Vec<TypedName>) =
        ...
        rule struct_body() -> Vec<TypedName> =
            [t!(Colon)] n:struct_entry() {
                n
            }

            rule struct_entry() -> Vec<TypedName> =
                [bb@t!(BeginBlock)] [t!(LineEnd)]? n:typed_name() ** [t!(LineEnd)] [t!(LineEnd)]? [be@t!(EndBlock)] {
                n
            }
        ...
        rule paren_name_list() -> (Vec<TypedName>, Option<TypedName>) =

            [t!(LParen)] n:typed_name()**[t!(Comma)] [t!(Comma)] v:vararg() [t!(Comma)]?  [t!(RParen)] {(n, Some(v))}
            /
            [t!(LParen)] n:typed_name()**[t!(Comma)] [t!(Comma)]?  [t!(RParen)] {(n, None)}
            /
            [t!(LParen)] v:vararg() [t!(Comma)]? [t!(RParen)] {(vec![], Some(v))}

        rule vararg() -> TypedName =
            [t!(Star)] n: typed_name() {n}

        rule maybe_arguments_and_equals() -> (Vec<TypedName>, Option<TypedName>, Option<TypeMention>) =
            n:paren_name_list() type_:type_annotation()? [t!(Equals)] {
                (n.0, n.1, type_)
            }
            / type_:type_annotation()? [t!(Equals)] {(Vec::new(), None, type_)}
        ...
        rule typed_name() -> TypedName =
            n:name() type_:type_annotation()? {
                TypedName {name:n, type_name: type_ }
            }
    }
}
